...
?>
	  ...
	  $url = 'https://spreadsheets.google.com/feeds/list/1DfqbxCNBjXuTiCujaSa-Ze4xYDA5VpHQ1LSvFmPFuD0/ou9p8c5/public/values?alt=json';
	  ...
	  foreach($rows as $row) {
		...
		if(substr($erste,0,6) === "Gruppe" Or substr($erste, -9) === "Hinspiele" Or substr($erste,0,6) === "Finale"){
			$i = 0;
			echo '</tbody></table></div></li>';			
		};
		if($i === 0){
			//Hin bzw. Rückspiele nach dem Begriff entfernen
			if(substr($erste, -9) === "Hinspiele"){
				$erste = before(' Hin', $erste);
			}else if(substr($erste, -9) === "Rückspiele"){
			    $erste = before(' Rück', $erste);
			};
