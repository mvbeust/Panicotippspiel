<?php
      ...
      $cachefile = "cache/BLATgames.html";
...
<?php
	  ...
	  $url = 'https://spreadsheets.google.com/feeds/list/1DfqbxCNBjXuTiCujaSa-Ze4xYDA5VpHQ1LSvFmPFuD0/op0co33/public/values?alt=json';
	  ...
	  foreach($rows as $row) {
		...
		if(substr($erste, -8) === "Spieltag" Or substr($erste,0,8) === "Playoffs"){
			$i = 0;
		...
		if($i === 1){
			if(strtotime($erste) > strtotime('-4 days') && strtotime($erste) < strtotime('+3 days')){
			 $active = "active";
			}else{
				$active = '';
			};
		  echo '<li><div class="collapsible-header ' . $active . '" style="font-weight: normal">' . $spieltag . ' (' . $erste . ')</div><div class="collapsible-body" style="padding: 20px;  margin: 0; background-color:#F5F5F5">';
